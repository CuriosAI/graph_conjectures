...
class Node:
    ...
    def select(self,idx,iter):
        ...
        #s =  open(f"{iter}ucb_values{idx}.txt","a")
        #s.write(f"UCB = {best_ucb} \n Graph \n {best_child.game.graph}\n")
        #s.close()
    ...
    def expand(self, policy):
        for index, prob in enumerate(policy):
            ...
            if prob > 0:
                # 0: ESTRARRE L'AZIONE: OGNI GIOCO HA UN ENCODING DIVERSO
                action = self.game.extract_action(index)

                # 1: copiare il gioco del padre
                child_game = copy.deepcopy(self.game)

                # 2 : aggiornare lo stato (fare lo step)
                # print('Pre expand update: {0}'.format(self.game.state))
                child_game.step(action) # tutta l'informazione su quanto avvenuto è dentro child_game
                # print('Post expand update: {0}'.format(self.game.state))
                # 3 : crea il nodo figlio
                child = Node(child_game, # gioco
                             self.args,  # args
                             child_game.state_to_observation(), # stato del figlio
                             self,   # nodo padre
                             #action, # action_taken
                             index,    # action_taken... ???
                             prob, # prior = policy
                ...
                # child_state = self.game.step(child_state, action, 0)[0]
                # child_state e 0 sono argomenti superflui
                # step prende in input solo l'azione
