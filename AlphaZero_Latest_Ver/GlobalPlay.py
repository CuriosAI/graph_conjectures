...
n = 4
stop = 3*int(n*(n-1)/2)
game = GlobalEnv(n,wagner_score,stop,normalize_reward=False)
...
play = open(f"GlobalMatch_on_{game.number_of_nodes}_{current_time.day}_{current_time.hour}_{current_time.minute}.txt", "w")
play = open(f"GlobalMatch_on_{game.number_of_nodes}_{current_time.day}_{current_time.hour}_{current_time.minute}.txt", "a")
play.write(f"MATCH ON {game.number_of_nodes} NODES WITH GLOBALENV. Stop after {stop} rounds\n")
...
while True:
    ...
    play.write(f"Round {game.timestep_it} - Action {action} --> ")
    if action >= game.number_of_edges:
        play.write("Flip edge ")
        action -= game.number_of_edges
    else:
        play.write("Leave unchanged edge ")
    i = action // game.number_of_nodes
    j = action % game.number_of_nodes
    play.write(f"({i},{j})\n")
...
g = open(f"Global_FinalGraph_{current_time.day}_{current_time.hour}_{current_time.minute}.g6", "w")
g = open(f"Global_FinalGraph_{current_time.day}_{current_time.hour}_{current_time.minute}.g6", "w")
#ipazia
