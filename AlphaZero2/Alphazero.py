...
class AlphaZero:
    ...
    # model: A neural network model that takes a game state as input and outputs a policy distribution over the possible moves and a value estimate for the current state.
    # optimizer: An optimizer that updates the weights of the neural network model based on the training data.
    # game: The game environment that the AlphaZero agent is playing.
    # args: A dictionary containing the hyperparameters for the AlphaZero algorithm.
    ...
    # 2. Set the initial player: Assign the current player to 1 (arbitrary choice) and start the game from the initial state.
    ...
    def selfPlay(self):
        ...
        memory = []
        # Set the current player as 1 (arbitrary choice)
        player = 1
        ...
        while True:
            ...
            if is_terminal:
                # Convert the memory data into a format suitable for training the neural network
                returnMemory = []
    ...
    # 4. Policy Probabilities: A vector of probabilities representing the likelihood of choosing each possible move from the current state.
    # 5. Value Estimate: A scalar value representing the estimated value of the current state, indicating whether the current player is likely to win or lose.
    # 6. Calculate the cross-entropy loss between the predicted policy probabilities and the target policy probabilities from the batch data. 
    ...
    # 7. Backpropagation and Weight Updates: Use the calculated losses to backpropagate through the neural network. 
    ...
    # 8. Repeat the Training Loop: After processing each batch of data, repeat the process until the specified number of epochs is reached. 
